...
export default function AdminOperatorsScreen() {
  ...
  useEffect(() => {
    supabase.from('operator_registrations').select('*').order('registered_at', { ascending: false }).limit(200)
      .then(({ data }) => { setOps(data ?? []); setLoading(false); });
  }, []);

  const handleDelete = async (id: string) => {
    await supabase.from('operator_registrations').delete().eq('id', id);
  ...
  const filtered = ops.filter(o => {
    ...
    return !q || (o.business_name + o.service + o.service_area).toLowerCase().includes(q);
  });
  ...
  return (
    <View style={styles.root}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Operators</Text>
        <Text style={styles.topCount}>{filtered.length}</Text>
      </View>

      <TextInput style={styles.search} value={search} onChangeText={setSearch} placeholder="Search business, service, area…" placeholderTextColor="#475569" />

      <ScrollView contentContainerStyle={styles.list}>
        {loading
          ? <ActivityIndicator color="#c8931a" style={{ marginTop: 40 }} />
          : filtered.length === 0
            ? <Text style={styles.empty}>No operator registrations found.</Text>
            : filtered.map(o => {
                const isConfirm = confirmId === o.id;
                const equip = [o.equipment_year, o.equipment_make, o.equipment_model].filter(Boolean).join(' ') || '—';
                return (
                  <View key={o.id} style={styles.card}>
                    <View style={styles.cardHeader}>
                      <View style={{ flex: 1 }}>
                        <Text style={styles.cardPrimary}>{o.business_name}</Text>
                        <Text style={styles.cardSub}>{o.service}  ·  {o.rate_per_acre || '—'}/ac</Text>
                        <Text style={styles.cardDate}>Registered {fmt(o.registered_at)}</Text>
                      </View>
                      <View style={styles.serviceBadge}>
                        <MaterialCommunityIcons name="tractor" size={18} color="#c8931a" />
                      </View>
                    </View>
                    ...
                    <View style={styles.metaRow}>
                      <View style={styles.metaChip}>
                        ...
                        <Text style={styles.metaValue}>{equip}</Text>
                      </View>
                      <View style={styles.metaChip}>
                        <Text style={styles.metaLabel}>Service Area</Text>
                        <Text style={styles.metaValue}>{o.service_area || '—'}</Text>
                      </View>
                      {o.labour_rate ? (
                        <View style={styles.metaChip}>
                          <Text style={styles.metaLabel}>Labour</Text>
                          <Text style={styles.metaValue}>{o.labour_rate}</Text>
                        </View>
                      ) : null}
                    </View>
                    ...
                    {isConfirm ? (
                      <View style={styles.confirmRow}>
                        <Text style={styles.confirmText}>Delete this operator listing?</Text>
                        <TouchableOpacity style={styles.confirmYes} onPress={() => handleDelete(o.id)}>
                      ...
                      <TouchableOpacity style={styles.deleteBtn} onPress={() => setConfirmId(o.id)}>
...
const styles = StyleSheet.create({
  ...
  serviceBadge:   { width: 38, height: 38, borderRadius: 10, backgroundColor: '#0f172a', alignItems: 'center', justifyContent: 'center' },
